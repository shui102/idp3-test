...
class RM65Inference:
    def __init__(self,obs_horizon=2, action_horizon=8, device="gpu",
        ...
        self.joint_qpos = [0.0 for _ in range(7)]
        self.pose = [0.0 for _ in range(7)]
        ...
        self._init_tracker(api_token= "73ebeba4255b3cf2a3dc13368ac25191",
                            prompt_text="brown-cup. white-basket. yellow-cup",
    ...
    def reset(self,first_init=True):
        ...
        # cprint(f"obs_dict: {obs_dict}",color="red")
    ...
    def _execute_action_pose(self, action):
        ...
        try:
            ...
            target_rad_angle = self.rm_interface.ik_solver.move_to_pose_and_get_joints(target_pos, target_quat)
            ...
            try:
                ...
                if eepose_dict:
                    ...
                    if eepose is not None:
                        # ee_pose:[-0.374286, -0.054759, 0.313315, 3.111, -0.129, 2.938]
                        # 获取到的就是弧度，赋值给前6个维度
                        self.joint_qpos[:6] = eepose
                if gripper_dict:
                    gripper_state = gripper_dict['left_arm']
                    # 更新第7维（夹爪状态）
                    self.joint_qpos[6] = gripper_state
