...
class MaskPointCloudExtractor:
    ...
    def extract_pcs_from_frame(self, rgb_array, depth_array, qpos_array):
        ...
        for rgb_np, depth_np, qpos in zip(rgb_array, depth_array, qpos_array):
            mask_array,_ = self.gen_mask(rgb_np)
            ...
            if VISUALIZE_DEBUG:
                ...
                # 2. 填充坐标 (前3列)
                temp_pcd.points = o3d.utility.Vector3dVector(resampled_pcd[:, :3])
                
                # 3. 填充颜色 (后3列)
                # 注意：Open3D要求颜色范围是 [0, 1]，如果你的数据是 [0, 255] 需要除以 255.0
                # 这里的 gripper_color 是 [0.0, 0.0, 1.0]，所以假设已经是归一化的
                temp_pcd.colors = o3d.utility.Vector3dVector(resampled_pcd[:, 3:6])
                
                # 4. 添加坐标轴方便观察方向 (原点处，大小为0.1米)
                coord_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1, origin=[0, 0, 0])
                
                print(f"正在显示点云，点数: {len(resampled_pcd)}")
                print("请关闭弹出的窗口以继续运行...")
                
                # 5. 启动可视化窗口
                o3d.visualization.draw_geometries([temp_pcd, coord_frame], 
