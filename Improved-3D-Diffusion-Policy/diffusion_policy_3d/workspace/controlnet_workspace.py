...
class iDP3ControlNetWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # two stage is only used for training, want inference, use stage2 instead
        if mode in ["stage1"]:
            _stage1 = cfg.stage1
            self.model = hydra.utils.instantiate(_stage1.policy)
            self.ema_model = copy.deepcopy(self.model) if _stage1.training.use_ema else None
            self.optimizer = hydra.utils.instantiate(_stage1.optimizer, params=self.model.parameters())
            self.ema = hydra.utils.instantiate(_stage1.ema, model=self.ema_model) if _stage1.training.use_ema else None

        if mode in ["stage2"]:
            _stage2 = cfg.stage2
            self.model = hydra.utils.instantiate(_stage2.policy)
            self.ema_model = copy.deepcopy(self.model) if _stage2.training.use_ema else None
    ...
    def get_model(self):
        ...
        if last_ckpt.is_file():
            ...
            self.load_checkpoint(path=last_ckpt,exclude_keys=["lr_scheduler"])
        policy = self.model
        if _stage.training.use_ema:
