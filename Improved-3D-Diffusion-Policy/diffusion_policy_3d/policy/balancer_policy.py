...
class BalancerPolicy(BasePolicy):
    def __init__(self, 
            shape_meta: dict,
    ...
    def compute_loss(self, batch, batch_weights=None):
        # normalize input
        ...
        if batch_weights is not None:
            weighted_loss = weighted_loss * batch_weights[:, None, None]
...
def main():
    ...
    batch = {
        "obs": {
            "point_cloud": torch.randn(B, T_obs, N_pts, 6, device=device),
            "agent_pos": torch.randn(B, T_obs, 10, device=device),
            "agent_rot": torch.randn(B, T_obs, 6, device=device),
        },
        "action": torch.randn(B, T_action, 30, device=device)  # (B, horizon, act_dim)
    }

    # --------------------------
    # 5. 调用 compute_loss
    # --------------------------
    total_loss, loss_dict = policy.compute_loss(batch)
